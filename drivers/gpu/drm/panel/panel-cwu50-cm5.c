...
#ifndef MIPI_DCS_EXIT_SLEEP_MODE
#define MIPI_DCS_EXIT_SLEEP_MODE 0x11
#endif

#ifndef MIPI_DCS_SET_DISPLAY_ON
...
static int cwu50_prepare(struct drm_panel *panel)
{
    struct cwu50 *ctx = panel_to_cwu50(panel);
    struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
    int ret, retry, attempt;
    u8 status;

    if (ctx->prepared)
        return 0;

    for (attempt = 0; attempt < 3; attempt++) {  // Allow up to 3 full initialization attempts
        dev_info(ctx->dev, "CWU50: Resetting panel... (Attempt %d)", attempt + 1);
        gpiod_set_value_cansleep(ctx->reset_gpio, 0);
        msleep(100);
        gpiod_set_value_cansleep(ctx->reset_gpio, 1);
        msleep(250);

        dev_info(ctx->dev, "CWU50: Running initialization sequence...");
        cwu50_init_sequence(ctx);
        msleep(150);

        /* Try exiting sleep mode */
        for (retry = 0; retry < 5; retry++) {
            dev_info(ctx->dev, "CWU50: Attempting to exit sleep mode (try %d/5)...", retry + 1);
            ret = mipi_dsi_dcs_exit_sleep_mode(dsi);
            if (ret == 0) {
                dev_info(ctx->dev, "CWU50: Successfully exited sleep mode");
                break;
            }
            dev_err(ctx->dev, "CWU50: Failed to exit sleep mode (%d), retrying...", ret);
            msleep(200);
        }
        if (ret) {
            dev_err(ctx->dev, "CWU50: Could not exit sleep mode after retries");
            continue;  // Restart initialization
        }
        msleep(300);

        /* Check for stable power mode (0x18) */
        ret = mipi_dsi_dcs_read(dsi, 0x0A, &status, 1);
        if (ret >= 0) {
            dev_info(ctx->dev, "CWU50: Panel power mode: 0x%02x", status);
            if (status == 0x18) {
                dev_info(ctx->dev, "CWU50: Panel power mode stable at 0x18");
                goto power_mode_success;
            }
        }
        dev_err(ctx->dev, "CWU50: Failed to read power mode on first attempt, restarting initialization...");
    }

    dev_err(ctx->dev, "CWU50: Panel failed initialization after multiple attempts");
    return -ENODEV;

power_mode_success:
    dev_info(ctx->dev, "CWU50: Turning display on...");
    ret = mipi_dsi_dcs_set_display_on(dsi);
    if (ret) {
        dev_err(ctx->dev, "CWU50: Failed to turn display on (%d)", ret);
        return ret;
    }
    msleep(150);
    ...
    /* Backlight enable */
    if (ctx->backlight) {
        dev_info(ctx->dev, "CWU50: Enabling backlight...");
        backlight_enable(ctx->backlight);
        msleep(50);
    } else {
        dev_err(ctx->dev, "CWU50: Backlight device not found!");
    }

    ctx->prepared = true;
    dev_info(ctx->dev, "CWU50: Panel successfully prepared");
    return 0;
}
